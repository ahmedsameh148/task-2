...
app.put('/member/:id',(req:any , res:any)=>{
    ...
    if(ok)res.send("The Member Was Edited Succecfully");
    else {res.status(400); res.send("Their Is No Member With That ID");}
});
...
app.delete('/member/:id',(req:any , res:any)=>{
    ...
    if(i==Members.length){res.status(400); res.send("Their Is No Member With That ID");}
    else {
        Members.splice(i,1); 
